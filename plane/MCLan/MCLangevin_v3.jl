...
function ev_en(l::Matrix{Float64},part::Matrix{Bool},occupied_neighbours::Matrix{Int},derivatives::Array{Vector{Float64}},x::Int,y::Int,k::Float64,k0::Float64,W::Float64,sigma::Float64)
	...
	if part[x,y]
		energy=k*energy-W*occupied_neighbours[x,y]
	else
		energy=k0*energy
	end	
	return energy*0.5*dA+log(dA)+sigma*dA
end
...
function run_sim(l::Matrix{Float64}, part::Matrix{Bool}, ld::Float64, N::Int,Nav::Int,Ninf::Int,kap::Float64,k0::Float64,W::Float64,sigma::Float64,phi_kd::Float64, Ne::Int,gamma::Float64,folder::String)
	...
	for sweep_MC in 1:N
		...
		if sweep_MC%Ninf==0
			...
			save(folder*"/measures.jld", "energy",measures[1:sweep_MC,1],"acc_MC",measures[1:sweep_MC,2],"kd",measures[1:sweep_MC,3],"average_cluster_size",measures[1:sweep_MC,4],"density",measures[1:sweep_MC,5])
...
function restart_sim(l::Matrix{Float64}, part::Matrix{Bool},Rkl::Array{Float64,3},last_kd::Float64, ld::Float64, N::Int,Nav::Int,Ninf::Int,kap::Float64,k0::Float64,W::Float64,sigma::Float64,phi_kd::Float64, Ne::Int,gamma::Float64,folder::String)
	L=Int(sqrt(length(part))) #this works for square lattices only
	measures=zeros(N,5)
	kds=zeros(N)
	#Rkl=zeros(Float64,L,L,4)
	...
	for sweep_MC in 1:N
		...
		for cl in clusters
			...
			if length(cl)>=Ne
				...
				for el in cl
					part[el[1],el[2]]=false
					@views for el in neig[el[1],el[2],1:4]
						occ_neig[el[1],el[2]]-=1
					end
				end
			end
